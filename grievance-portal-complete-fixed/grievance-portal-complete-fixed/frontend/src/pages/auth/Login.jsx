...
const GOOGLE_CLIENT_ID = '141818306291-f46qjb1psh5gh8s1truf6rj1tjl37j84.apps.googleusercontent.com';
...
export default function Login() {
  ...
  // Stable Google credential callback using ref to avoid stale closures
  const googleLoginRef = useRef(googleLogin);
  const navigateRef = useRef(navigate);
  useEffect(() => { googleLoginRef.current = googleLogin; }, [googleLogin]);
  useEffect(() => { navigateRef.current = navigate; }, [navigate]);

  // Initialize Google Identity Services — once only
  useEffect(() => {
      ...
      window.google.accounts.id.initialize({
        ...
        callback: async (response) => {
          ...
          try {
            ...
            navigateRef.current(REDIRECT_MAP[user.role] || '/dashboard');
          } catch (err) {
    ...
    if (!doInit()) {
      intervalId = setInterval(() => {
        if (doInit() && intervalId) clearInterval(intervalId);
      }, 500);
    }
            ...
            >
              ...
              <div className="w-full relative z-10 flex justify-center">
                ...
                >
                  ...
                  <span>{googleProcessing ? 'Signing in...' : 'Continue with Google'}</span>
                </button>
              </div>
